...
enum optype {var,str, label, move, addi,addr, subi,subr, muli,mulr, divi,divr, 
	     ...
	     sys, endofprogram, eemptyline, unknown };
	}
enum operandtype {id,stackref,reg,num,strval,eempty,nonknown};
...
public:
  ...
  void print(){
    ...
    case eempty: {if (debug>=4) cout<<"eemptyop"; break;}
...
//if (ch!='\n' && !src.eof()) {cout<<"warning: '"<<(int) ch<<"' found at end of line "<<line<<'\n';}
  ...
  else if (ss=="") code = opcodes::eemptyline;
...
int main(int argc, char *argv[]){
  ...
  while (!done) {
    ...
    else if (code==opcodes::eemptyline) {}
    else {
      //      checkoperands(op1,op2); 
      // op1 can be num/reg/id, op2 can be reg/id/strval/eempty
	...
	passert(op2.type()==eempty,"only one operand expected"); 
	...
	break;
      ...
      case opcodes::push: {
	passert(op2.type()==eempty,"zero or one operand expected"); 
	passert(op1.type()==num || op1.type()==reg || op1.type()== id
		|| op1.type()== stackref || op1.type()==eempty ,
		"illegal operand type"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
	...
	break;
      ...
      case opcodes::ret: {
	passert(op1.type()==eempty && op2.type()==eempty,"no operand expected"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
      case opcodes::link: {
	passert(op1.type()==num && op2.type()==eempty,"illegal operand"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
